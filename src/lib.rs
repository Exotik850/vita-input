//! # vita-input
//!
//! An idiomatic Rust wrapper around the Vita's `SceCtrl` input polling functions
//! from [`vitasdk-sys`].
//!
//! ## Quick start
//!
...
//! ```
//!
//! By default the library sets the sampling mode to **Analog** so that both
//! digital buttons and analogue sticks are available.  Call
//! [`VitaInput::set_sampling_mode`] before your first poll if you need a
//! different mode.
...
impl VitaInput {
    ...
    #[inline]
    pub fn joystick(&self, which: Joystick) -> JoystickData {
        match which {
